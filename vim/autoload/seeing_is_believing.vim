...
function! seeing_is_believing#myrun(mode) range "{{{
  if     a:mode == 'n'
    let range_str = '%'
  elseif a:mode == 'v'
    let range_str = "'<,'>"
  ...
  let wintop_pos = getpos('w0')
  set lazyredraw
  execute ":" . range_str . "!seeing_is_believing"
  call setpos('.', wintop_pos)
  normal! zt
  call setpos('.', cursor_pos)
  set nolazyredraw
...
function! seeing_is_believing#clear(mode) range "{{{
  if     a:mode == 'n'
    let range_str = '%'
  elseif a:mode == 'v'
    let range_str = "'<,'>"
  ...
  let wintop_pos = getpos('w0')
  set lazyredraw
  execute ":" . range_str . "!seeing_is_believing -c"
  call setpos('.', wintop_pos)
  normal! zt
  call setpos('.', cursor_pos)
  set nolazyredraw
